...
export const api = {
  ...
  async createOverviewJob(token: string, customer: string): Promise<any> {
    try {
      ...
      console.log('📡 Create Overview Job Response status:', response.status, response.statusText)

      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Create Overview Job Error:', errorText)
        throw new Error(`HTTP error! status: ${response.status} - ${errorText}`)
      }

      const data = await response.json()
      console.log('📦 Create Overview Job Response data:', data)
      return data
    } catch (error) {
      console.error('Create Overview Job error:', error)
      throw new Error('Erro ao criar job de visão geral.')
    }
  },

  async getOverviewJobStatus(token: string, jobId: string): Promise<any> {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Get Overview Job Status Error:', errorText)
        throw new Error(`HTTP error! status: ${response.status} - ${errorText}`)
      }
      ...
      console.error('Get Overview Job Status error:', error)
      throw new Error('Erro ao verificar status do job de visão geral.')
    }
  },

  async getOverviewData(token: string, jobId: string): Promise<OverviewResponse> {
    try {
      const API_V2_URL = `https://clownfish-app-l84ar.ondigitalocean.app/api/request/${jobId}/data`
      
      console.log('🌐 Get Overview Data Request:', {
        url: API_V2_URL,
        method: 'GET'
      })
      ...
      console.log('📡 Get Overview Data Response status:', response.status, response.statusText)
      console.log('📡 Get Overview Data Response headers:', response.headers.get('content-type'))

      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Get Overview Data Error:', errorText)
      ...
      console.log('📦 Get Overview Data Response data:', data)
      ...
      throw new Error('Erro ao buscar dados de visão geral.')
    }
  },
  ...
  async createProductsFunnelJob(token: string, customer: string, startDate?: string, endDate?: string): Promise<any> {
    try {
      const API_V2_URL = 'https://clownfish-app-l84ar.ondigitalocean.app/api/request'
  ...
  async getProductsFunnelJobStatus(token: string, jobId: string): Promise<any> {
    try {
      const API_V2_URL = `https://clownfish-app-l84ar.ondigitalocean.app/api/request/${jobId}`
  ...
  async getProductsFunnelData(token: string, jobId: string): Promise<any> {
    try {
      const API_V2_URL = `https://clownfish-app-l84ar.ondigitalocean.app/api/request/${jobId}/data`
  ...
  async createWhatsAppFunnelJob(token: string, customer: string, startDate?: string, endDate?: string): Promise<any> {
    try {
      const API_V2_URL = 'https://clownfish-app-l84ar.ondigitalocean.app/api/request'
  ...
  async getWhatsAppFunnelJobStatus(token: string, jobId: string): Promise<any> {
    try {
      const API_V2_URL = `https://clownfish-app-l84ar.ondigitalocean.app/api/request/${jobId}`
  ...
  async getWhatsAppFunnelData(token: string, jobId: string): Promise<any> {
    try {
      const API_V2_URL = `https://clownfish-app-l84ar.ondigitalocean.app/api/request/${jobId}/data`
  ...
  // API 2.0 - Influencers (iwannasleep)
  // Sempre busca os últimos 90 dias automaticamente
  async createInfluencersJob(token: string, customer: string): Promise<any> {
    try {
      const API_V2_URL = 'https://clownfish-app-l84ar.ondigitalocean.app/api/request'
      ...
      console.log('🌐 Create Influencers Job Request:', {
        url: API_V2_URL,
        method: 'POST',
        customer,
        note: 'Busca automática dos últimos 90 dias'
      ...
      const response = await fetch(API_V2_URL, {
        ...
        body: JSON.stringify({
          endpoint: 'iwannasleep/influencers',
          params
        }),
      })

      console.log('📡 Create Influencers Job Response status:', response.status, response.statusText)

      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Create Influencers Job Error:', errorText)
        throw new Error(`HTTP error! status: ${response.status} - ${errorText}`)
      }

      const data = await response.json()
      console.log('📦 Create Influencers Job Response data:', data)
      return data
    } catch (error) {
      console.error('Create Influencers Job error:', error)
      throw new Error('Erro ao criar job de influencers.')
    }
  },

  async getInfluencersJobStatus(token: string, jobId: string): Promise<any> {
    try {
      const API_V2_URL = `https://clownfish-app-l84ar.ondigitalocean.app/api/request/${jobId}`
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Get Influencers Job Status Error:', errorText)
        throw new Error(`HTTP error! status: ${response.status} - ${errorText}`)
      }
      ...
      console.error('Get Influencers Job Status error:', error)
      throw new Error('Erro ao verificar status do job de influencers.')
    }
  },

  async getInfluencersData(token: string, jobId: string): Promise<any> {
    try {
      const API_V2_URL = `https://clownfish-app-l84ar.ondigitalocean.app/api/request/${jobId}/data`
      
      console.log('🌐 Get Influencers Data Request:', {
        url: API_V2_URL,
        method: 'GET'
      })
      ...
      console.log('📡 Get Influencers Data Response status:', response.status, response.statusText)
      console.log('📡 Get Influencers Data Response headers:', response.headers.get('content-type'))

      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Get Influencers Data Error:', errorText)
      ...
      // Substituir NaN por null antes de fazer parse
      const cleanedText = text.replace(/:\s*NaN\b/g, ': null')
      ...
      console.log('📦 Get Influencers Data Response data:', data)
      ...
      console.error('Get Influencers Data error:', error)
      if (error instanceof Error) {
        throw error
      }
      throw new Error('Erro ao buscar dados de influencers.')
    }
  }
} 